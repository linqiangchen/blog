<template>
  <div id="myapp">
    ...
    <el-carousel :interval="2000" type="card" height="400px">
      <el-carousel-item v-for="item in data" :key="item.id" class="liti">
        <h1 class="card"></h1>
        <h1 class="card">{{item.con}}</h1>
...
export default {
  ...
  created() {
    ...
    this.$axios
      .get("http://10.20.158.17:3000/artic")
      .then((res) => {
        this.data = res.data;
      })
      .catch((err) => {});
...
.card {
  ...
  /* background-color: rgba(29, 54, 52, 0.822) */
}
</style>
